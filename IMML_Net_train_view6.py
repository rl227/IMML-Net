...
from IMML_Net_view6 import RSSLRNet
...
def train(model, epoch, lr, alpha, beta, n_cluster):
    ...
    for i in range(epoch):
        ...
        loss_Self1 = criterion(output_h, features[0].t().mm(features[0])) + criterion(output_h, features[1].t().mm(features[1])) + criterion(output_h, features[2].t().mm(features[2])) + \
                     criterion(output_h, features[3].t().mm(features[3])) + criterion(output_h, features[4].t().mm(features[4])) + criterion(output_h, features[5].t().mm(features[5]))
        loss_Self2 = criterion(output_h, adj[0]) + criterion(output_h, adj[1]) + criterion(output_h, adj[2]) + criterion(output_h, adj[3]) + criterion(output_h, adj[4]) + criterion(output_h, adj[5])
        ...
        # scipy.io.savemat('Caltech101-7_Z.mat', mdict={'output_hh': output_hh})
        # [ACC, NMI, Purity, ARI, Fscore, Precision, Recall] = spectral_clustering((output_hh + output_hh.T)/2, n_cluster, labels, repnum=10)
    ...
    # sio.savemat('./per_loss1/Caltech101-7_1_clustering_per_.mat', {'idx': per_})
    # sio.savemat('./per_loss1/Caltech101-7_1_clustering_loss_total.mat', {'idx': loss_list})

    alpha_str = str(alpha).replace('.', '_')
    beta_str = str(beta).replace('.', '_')

    sio.savemat(f'./per_loss1/Youtube_1_clustering_per_alpha{alpha_str}_beta{beta_str}.mat', {'idx': per_})
    sio.savemat(f'./per_loss1/Youtube_1_clustering_loss_total_alpha{alpha_str}_beta{beta_str}.mat',
                {'idx': loss_list})
...
if __name__ == '__main__':
    ...
    for ii in range(len(alpha_list)):
        for j in range(len(beta_list)):
            ...
            datasetW_dir = "./datasetW"
            adj = []
            dv = []
            Y1 = []
            # MG0 = loadMGW1(os.path.join(datasetW_dir, "Caltech101all1GW.mat"))  # 单个视角拉普拉斯矩阵LG
            # MG1 = loadMGW2(os.path.join(datasetW_dir, "Caltech101all2GW.mat"))  # 单个视角拉普拉斯矩阵LG
            # MG2 = loadMGW3(os.path.join(datasetW_dir, "Caltech101all3GW.mat"))  # 单个视角拉普拉斯矩阵LG
            # MG3 = loadMGW4(os.path.join(datasetW_dir, "Caltech101all4GW.mat"))  # 单个视角拉普拉斯矩阵LG
            # MG4 = loadMGW5(os.path.join(datasetW_dir, "Caltech101all5GW.mat"))  # 单个视角拉普拉斯矩阵LG
            # MG5 = loadMGW6(os.path.join(datasetW_dir, "Caltech101all6GW.mat"))  # 单个视角拉普拉斯矩阵LG
            #
            # adj.append(MG0)  # - 0.2
            # adj.append(MG1)  # - 0.2
            # adj.append(MG2)  # - 0.2
            ...
            adj_tmp = loadMGW(os.path.join(datasetW_dir, "YoutubeMGW.mat"))  # 单个视角拉普拉斯矩阵LG
            adj.append(adj_tmp[0][0])  # - 0.2
            adj.append(adj_tmp[0][1])  # - 0.2
            adj.append(adj_tmp[0][2])  # - 0.2
            adj.append(adj_tmp[0][3])  # - 0.2
            adj.append(adj_tmp[0][4])  # - 0.2
            adj.append(adj_tmp[0][5])  # - 0.2

            adj1, features, labels, view_count = features_to_adj()

            adj_all = adj[0] + adj[1] + adj[2] + adj[3] + adj[4] + adj[5]
