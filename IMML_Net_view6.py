...
class RSSLRNet(nn.Module):
    def __init__(self, features, adj, Y1, Y2, u0, umax, p0, e1, e2, y1, n, n_cluster, dv, blocks, para, use_cuda):
        ...
        #     # self.beta1.append(nn.Parameter(torch.ones(self.n, self.n, dtype=torch.float32)))
        #     # self.beta2.append(nn.Parameter(torch.ones(self.n, self.n, dtype=torch.float32)))
        #
        #     self.fc_z.append(nn.Linear(self.n, self.n, bias=False))
        # for i in range(len(self.X)):
        #     self.fc_v.append(nn.Linear(self.n, self.dv[i], bias=False))
        ...
        self.lag4 = nn.Linear(self.n, self.n, bias=False)
        self.lag5 = nn.Linear(self.n, self.n, bias=False)
        self.lag6 = nn.Linear(self.n, self.n, bias=False)
    ...
    # def self_active_sh(self, u, thershold):
    #     tmp_u = torch.abs(u)
    #     h = torch.abs(tmp_u - thershold).mul(torch.sign(u))
    #     return h
    ...
    def forward(self, X, adj):
        ...
        Z_init = self.con2(self.bn_input_01(adj[0] + adj[1] + adj[2] + adj[3] + adj[4] + adj[5]))
        ...
        E_init_tmp_4 = X[3] - X[3].mm(Z[-1])
        # E4.append(self.self_active_sh(E_init_tmp_4, self.active_para2))
        E_init_tmp_4_zero = torch.zeros_like(E_init_tmp_4)
        for j in range(E_init_tmp_4.size(1)):
            E_init_tmp_tmp_4 = E_init_tmp_4[:, j]
            E_init_tmp_4_zero[:, j] = self.self_active_l21(E_init_tmp_tmp_4, self.active_para2)
        E4.append(E_init_tmp_4_zero)

        E_init_tmp_5 = X[4] - X[4].mm(Z[-1])
        # E5.append(self.self_active_sh(E_init_tmp_5, self.active_para2))
        E_init_tmp_5_zero = torch.zeros_like(E_init_tmp_5)
        for j in range(E_init_tmp_5.size(1)):
            E_init_tmp_tmp_5 = E_init_tmp_5[:, j]
            E_init_tmp_5_zero[:, j] = self.self_active_l21(E_init_tmp_tmp_5, self.active_para2)
        E5.append(E_init_tmp_5_zero)

        E_init_tmp_6 = X[5] - X[5].mm(Z[-1])
        # E6.append(self.self_active_sh(E_init_tmp_6, self.active_para2))
        E_init_tmp_6_zero = torch.zeros_like(E_init_tmp_6)
        for j in range(E_init_tmp_6.size(1)):
            E_init_tmp_tmp_6 = E_init_tmp_6[:, j]
            E_init_tmp_6_zero[:, j] = self.self_active_l21(E_init_tmp_tmp_6, self.active_para2)
        E6.append(E_init_tmp_6_zero)
        ...
        Y1_4.append(self.lag4(self.Y1[3] + self.u0 * X[3]))
        Y1_5.append(self.lag5(self.Y1[4] + self.u0 * X[4]))
        Y1_6.append(self.lag6(self.Y1[5] + self.u0 * X[5]))
        ...
        for k in range(self.blocks):
            ...
            Z_tmp = self.con1(self.bn_input_01(Z[-1])) + self.con2(self.bn_input_01(X[0].t().mm(X[0]) - X[0].t().mm(E1[-1]) + X[0].t().mm(Y1_1[-1] / uk[-1]) +
                     ...
                     X[2].t().mm(X[2]) - X[2].t().mm(E3[-1]) + X[2].t().mm(Y1_3[-1] / uk[-1]) +
                     X[3].t().mm(X[3]) - X[3].t().mm(E4[-1]) + X[3].t().mm(Y1_4[-1] / uk[-1]) +
                     X[4].t().mm(X[4]) - X[4].t().mm(E5[-1]) + X[4].t().mm(Y1_5[-1] / uk[-1]) +
                     X[5].t().mm(X[5]) - X[5].t().mm(E6[-1]) + X[5].t().mm(Y1_6[-1] / uk[-1]) + (W[-1] - Y2[-1] / uk[-1])))
            ...
            E_tmp_tmp_4 = X[3] - X[3].mm(Z[-1]) + Y1_4[-1] / uk[-1]
            E_tmp_tmp_4_zero = torch.zeros_like(E_tmp_tmp_4)
            for j in range(E_tmp_tmp_4.size(1)):
                E_tmp_tmp_tmp_4 = E_tmp_tmp_4[:, j]
                E_tmp_tmp_4_zero[:, j] = self.self_active_l21(E_tmp_tmp_tmp_4, self.active_para2)
            E4.append(E_tmp_tmp_4_zero)

            E_tmp_tmp_5 = X[4] - X[4].mm(Z[-1]) + Y1_5[-1] / uk[-1]
            E_tmp_tmp_5_zero = torch.zeros_like(E_tmp_tmp_5)
            for j in range(E_tmp_tmp_5.size(1)):
                E_tmp_tmp_tmp_5 = E_tmp_tmp_5[:, j]
                E_tmp_tmp_5_zero[:, j] = self.self_active_l21(E_tmp_tmp_tmp_5, self.active_para2)
            E5.append(E_tmp_tmp_5_zero)

            E_tmp_tmp_6 = X[5] - X[5].mm(Z[-1]) + Y1_6[-1] / uk[-1]
            E_tmp_tmp_6_zero = torch.zeros_like(E_tmp_tmp_6)
            for j in range(E_tmp_tmp_6.size(1)):
                E_tmp_tmp_tmp_6 = E_tmp_tmp_6[:, j]
                E_tmp_tmp_6_zero[:, j] = self.self_active_l21(E_tmp_tmp_tmp_6, self.active_para2)
            E6.append(E_tmp_tmp_6_zero)
            ...
            Y1_4.append(self.lag4(Y1_4[-1] + uk[-1] * (X[3] - X[3].mm(Z[-1]) - E4[-1])))
            Y1_5.append(self.lag5(Y1_5[-1] + uk[-1] * (X[4] - X[4].mm(Z[-1]) - E5[-1])))
            Y1_6.append(self.lag6(Y1_6[-1] + uk[-1] * (X[5] - X[5].mm(Z[-1]) - E6[-1])))
