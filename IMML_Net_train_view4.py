...
from IMML_Net_view4 import RSSLRNet
from utils import features_to_adj, features_to_adj_1, features_to_adj_2, features_to_adj_3, weights_init, plot, plot1, spectral_clustering
...
def train(model, epoch, lr, alpha, beta, n_cluster):
    ...
    for i in range(epoch):
        ...
        loss_Self1 = criterion(output_h, features[0].t().mm(features[0])) + criterion(output_h, features[1].t().mm(features[1])) + criterion(output_h, features[2].t().mm(features[2])) + \
                     criterion(output_h, features[3].t().mm(features[3]))
        loss_Self2 = criterion(output_h, adj[0]) + criterion(output_h, adj[1]) + criterion(output_h, adj[2]) + criterion(output_h, adj[3])
        ...
        scipy.io.savemat('ALOI_Z.mat', mdict={'output_hh': output_hh})
    ...
    #
    # sio.savemat('./per_loss1/BBC4view_clustering_per_.mat', {'idx': per_})
    # sio.savemat('./per_loss1/BBC4view_clustering_loss_total.mat', {'idx': loss_list})

    alpha_str = str(alpha).replace('.', '_')
    beta_str = str(beta).replace('.', '_')

    sio.savemat(f'./per_loss1/ALOI_1_clustering_per_alpha{alpha_str}_beta{beta_str}.mat', {'idx': per_})
    sio.savemat(f'./per_loss1/ALOI_1_clustering_loss_total_alpha{alpha_str}_beta{beta_str}.mat',
                {'idx': loss_list})
...
if __name__ == '__main__':
    ...
    for ii in range(len(alpha_list)):
        for j in range(len(beta_list)):
            ...
            adj_tmp = loadMGW(os.path.join(datasetW_dir, "ALOIMGW.mat"))  # 单个视角拉普拉斯矩阵LG
            adj.append(adj_tmp[0][0])  # - 0.2
            adj.append(adj_tmp[0][1])  # - 0.2
            adj.append(adj_tmp[0][2])  # - 0.2
            adj.append(adj_tmp[0][3])  # - 0.2

            adj, features, labels, view_count = features_to_adj()

            adj_all = adj[0] + adj[1] + adj[2] + adj[3]
